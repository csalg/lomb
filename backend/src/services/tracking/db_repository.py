...
from lib.db import get_db
 ...
 # log_repository_constructor = Logger,
# ignore_repository_constructor = None,
# learning_repository_constructor = None):
...
class SetRepository(ABC):
    ...
    def add(self,key):
        self.collection.insert_one({"_id": key})

    def delete(self,key):
        self.collection.delete_many({"_id":key})

    def find(self,key):
        return self.collection.find({'id':key})
